...
import CRMAccommodationAssistant from '../../../../../src/v4/crm/accommodation-assistant/accommodation-assistant';
...
describe('CRMAccommodationAssistant', () => {
  describe('new CRMAccommodationAssistant', () => {
    test('GIVEN a SARASAccommodationAssistant WHEN called THEN a new CRMAccommodationAssistant is created', () => {
      ...
      const expectedAccommodation = new CRMAccommodationAssistant(accommodation, '1234', '/teams(test-owner-id)');

      expect(expectedAccommodation['ftts_Testhistory@odata.bind']).toBe('1234');
      expect(expectedAccommodation.ftts_accommodationtype).toBe('2,3,4');
      ...
      expect(expectedAccommodation['ownerid@odata.bind']).toBe('/teams(test-owner-id)');
    });
