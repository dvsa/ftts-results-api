...
import { CRMTestItem } from '../../../../../src/v4/crm/test-item/test-item';
import { CRMItemTypes } from '../../../../../src/v4/crm/test-item/item-type';
...
describe('CRMTestItem', () => {
  describe('new CRMTestItem', () => {
    test('GIVEN a SARASItem WHEN called THEN a new CRMTestItem is created', () => {
      ...
      const expectedItem: CRMTestItem = new CRMTestItem(item, '1234', '/teams(test-owner-id)');
      ...
      expect(expectedItem.ftts_name).toBe('Survey - 2 - test-code');
      ...
      expect(expectedItem.ftts_userresponses).toBe('1,2,3');
