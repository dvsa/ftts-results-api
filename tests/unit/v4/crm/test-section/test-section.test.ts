...
import { CRMTestSection } from '../../../../../src/v4/crm/test-section/test-section';
import { CRMSectionType } from '../../../../../src/v4/crm/test-section/section-type';
...
describe('CRMTestSection', () => {
  describe('new CRMTestSection', () => {
    test('GIVEN a SARASSection WHEN called THEN a new CRMTestSection is created', () => {
      ...
      const expectedItem: CRMTestSection = new CRMTestSection(section, 'test-history-id', '/teams(test-owner-id)', CRMSectionType.HPT_TEST_RESULT);

      expect(expectedItem.ftts_name).toBe('HPTTestResult - 0 - Road Signs');
      ...
      expect(expectedItem['ownerid@odata.bind']).toBe('/teams(test-owner-id)');
      expect(expectedItem['ftts_Testhistory@odata.bind']).toBe('test-history-id');
    });
  });
});
